...
export default class Registration extends Vue {
  @Action(namespaced(NS_ANALYTICS, FETCH_REGISTRATION_REPORTS))
  fetchRegistrationReports;
  @Action(namespaced(NS_ANALYTICS, FETCH_REGISTRATION_TOTAL))
  ...
  async handleFetchReports() {
    ...
    await this.fetchRegistrationReports(params).then((data) => {
      this.analytics = data.results;
      this.count = data.count;
    });

    await this.fetchRegistrationTotal(params).then((data) => {
  ...
  async asyncData({ route, $axios, store, error, $dayjs }) {
    ...
    const getAnalytics = await store
      .dispatch(namespaced(NS_ANALYTICS, FETCH_REGISTRATION_REPORTS), params)
      .then((data) => {
    ...
    const getTotal = await store
      .dispatch(namespaced(NS_ANALYTICS, FETCH_REGISTRATION_TOTAL), params)
      .then((data) => {
        mediaTotal = data;
      })
      .catch((e) => {
