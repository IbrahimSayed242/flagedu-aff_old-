...
export default class Media extends Vue {
  @Action(namespaced(NS_ANALYTICS, FETCH_MEDIA_REPORTS)) fetchMediaReports;
  @Action(namespaced(NS_ANALYTICS, FETCH_MEDIA_TOTAL)) fetchMediaTotal;
  ...
  async handleFetchReports() {
    ...
    await this.fetchMediaReports(params).then((data) => {
      this.analytics = data.results;
      this.count = data.count;
    });

    await this.fetchMediaTotal(params).then((data) => {
  ...
  async asyncData({ route, $axios, store, error, $dayjs }) {
    ...
    const getAnalytics = await store
      .dispatch(namespaced(NS_ANALYTICS, FETCH_MEDIA_REPORTS), params)
      .then((data) => {
    ...
    const getTotal = await store
      .dispatch(namespaced(NS_ANALYTICS, FETCH_MEDIA_TOTAL), params_get_total)
      .then((data) => {
        mediaTotal = data;
      })
      .catch((e) => {
