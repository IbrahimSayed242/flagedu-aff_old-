  ...
  <div>
    ...
    <div class="filters">
      ...
      <div class="stat-items">
        ...
        <div class="stat-item">
          <div class="result">
            ${{ userDetail.balance ? userDetail.balance : 0 }}
          </div>
...
import { NS_ANALYTICS, NS_SUBAFFILIATE } from "@/utils/store/namespace.names";
import {
...
export default class UserDetail extends Vue {
  @Action(namespaced(NS_ANALYTICS, FETCH_ADMIN_DASHBOARD_ANALYTICS_SUMMARY))
  fetchAdminDashboardAnalyticsSummary;
  ...
  async handleFetchReports() {
    ...
    await this.fetchAdminDashboardAnalyticsSummary(params).then((data) => {
  ...
  async asyncData({ route, $axios, store, error, $dayjs }) {
    ...
    const getAnalytics = await store
      .dispatch(
        namespaced(NS_ANALYTICS, FETCH_ADMIN_DASHBOARD_ANALYTICS_SUMMARY),
        params
      )
      .then((data) => {
        summaryData = data;
